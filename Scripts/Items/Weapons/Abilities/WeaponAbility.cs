...
namespace Server.Items
{
	public abstract class WeaponAbility
	{
		...
		public virtual int CalculateMana( Mobile from )
		{
			...
			if ( GetSkill(from, SkillName.Magery) < 60 ) //Magery check added by Blady
			{
				if ( skillTotal >= 300.0 )
					mana -= 10;
			...
			*/
			...
			if ( context != null )
				mana = mana * 2 * context.Consecutives;
			//end edit
			return mana;
		}
		...
		private class WeaponAbilityContext
		{
			...
			public int Consecutives;//added by Sunny

			public Timer Timer{ get{ return m_Timer; } }

			public WeaponAbilityContext( Timer timer, int consecs  )
			{
				m_Timer = timer;
				Consecutives = consecs;
			}
		}
	}
}