...
{
	public class BannerOSI : Item, IAddon, IRewardItem
	{
		//TODO: Replace all Banner with BannerOSI then convert the class name back
		...
		public Item Deed
		{
			get
			{
				BannerOSIDeed deed = new BannerOSIDeed();
		...
		[Constructable]
		public BannerOSI( int itemID ) : base( itemID )
		{
			LootType = LootType.Blessed;
			Movable = false;
		}

		public BannerOSI( Serial serial ) : base( serial )
		{
		}
	...
	public class BannerOSIDeed : Item, IRewardItem
	{
		...
		[Constructable]
		public BannerOSIDeed() : base( 0x14F0 )
		{
			LootType = LootType.Blessed;
			Weight = 1.0;
		}

		public BannerOSIDeed( Serial serial ) : base( serial )
		{
		}
		...
		private class InternalGump : Gump
		{
			...
			private BannerOSIDeed m_Banner;

			public InternalGump( BannerOSIDeed banner ) : base( 100, 200 )
			{
				m_Banner = banner;
		...
		private class InternalTarget : Target
		{
			...
			public InternalTarget( BannerOSIDeed banner, int itemID ) : base( -1, true, TargetFlags.None )
			{
				m_Banner = banner;
				m_ItemID = itemID;
			}
					...
					if ( house != null && house.IsOwner( from ) )
					{
						...
						if ( map.CanFit( p3d, id.Height ) )
						{
							...
							if ( house != null && house.IsOwner( from ) )
							{
								...
								{
									BannerOSI banner = null;

									if ( north )
										banner = new BannerOSI( m_ItemID );
									else if ( west )
										banner = new BannerOSI( m_ItemID + 1 );
			...
			private class FacingGump : Gump
			{
				...
				public FacingGump( BannerOSIDeed banner, int itemID, Point3D location, BaseHouse house ) : base( 150, 50 )
				{
					m_Banner = banner;
					m_ItemID = itemID;
					m_Location = location;
