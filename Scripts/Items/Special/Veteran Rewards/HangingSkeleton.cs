...
{
	public class HangingSkeletonOSI : Item, IAddon, IRewardItem
	{
		...
		public Item Deed
		{
			get
			{
				HangingSkeletonOSIDeed deed = new HangingSkeletonOSIDeed();
		...
		[Constructable]
		public HangingSkeletonOSI() : this( 0x1596 )
		{
		}

		[Constructable]
		public HangingSkeletonOSI( int itemID ) : base( itemID )
		{
			LootType = LootType.Blessed;
			Movable = false;
		}

		public HangingSkeletonOSI( Serial serial ) : base( serial )
		{
		}
	...
	public class HangingSkeletonOSIDeed : Item, IRewardItem
	{
		...
		[Constructable]
		public HangingSkeletonOSIDeed() : base( 0x14F0 )
		{
			LootType = LootType.Blessed;
			Weight = 1.0;
		}

		public HangingSkeletonOSIDeed( Serial serial ) : base( serial )
		{
		}
		...
		{
			private HangingSkeletonOSIDeed m_Skeleton;

			public InternalGump( HangingSkeletonOSIDeed skeleton ) : base( 100, 200 )
			{
				m_Skeleton = skeleton;
		...
		private class InternalTarget : Target
		{
			...
			public InternalTarget( HangingSkeletonOSIDeed banner, int itemID ) : base( -1, true, TargetFlags.None )
			{
				m_Skeleton = banner;
				m_ItemID = itemID;
			}
					...
					if ( house != null && house.IsOwner( from ) )
					{
						...
						if ( map.CanFit( p3d, id.Height ) )
						{
							...
							if ( house != null && house.IsOwner( from ) )
							{
								...
								{
									HangingSkeletonOSI banner = null;

									if ( north )
										banner = new HangingSkeletonOSI( m_ItemID );
									else if ( west )
										banner = new HangingSkeletonOSI( GetWestItemID( m_ItemID ) );
			...
			private class FacingGump : Gump
			{
				...
				public FacingGump( HangingSkeletonOSIDeed banner, int itemID, Point3D location, BaseHouse house ) : base( 150, 50 )
				{
					m_Skeleton = banner;
					m_ItemID = itemID;
					m_Location = location;
