...
namespace Server.Items
{
	public abstract class BaseRunicTool : BaseTool
	{
		...
		{
			return Scale( min, max, low, high, m_LuckChance );
		}

		public static int Scale( int min, int max, int low, int high, int luckchance )
		{
			int percent;
			...
			else
			{
				...
				if ( LootPack.CheckLuck( luckchance ) )
					v += (Utility.RandomMinMax(10, luckchance/100)); //Increased luck effect on intensity - by Blady; default was += 10
				if ( v > max )
					v = max;
		...
		//Edited by sunny: created the freeze bug
		...
		private static void ApplyResistance( BaseHat hat, int min, int max, ResistanceType res, int low, int high )
		{
			switch ( res )
			{
				case ResistanceType.Physical: hat.Resistances.Physical += Scale( min, max, low, high ); break;
				case ResistanceType.Fire: hat.Resistances.Fire += Scale( min, max, low, high ); break;
				case ResistanceType.Cold: hat.Resistances.Cold += Scale( min, max, low, high ); break;
				case ResistanceType.Poison: hat.Resistances.Poison += Scale( min, max, low, high ); break;
				case ResistanceType.Energy: hat.Resistances.Energy += Scale( min, max, low, high ); break;
			}
		}
		...
		public static void ApplyAttributesTo( BaseWeapon weapon, bool isRunicTool, int luckChance, int attributeCount, int min, int max )
		{
			...
			for ( int i = 0; i < attributeCount; ++i )
			{
				...
				switch ( random )
				{
					...
					case 16: ApplyAttribute( secondary,	min, max, AosWeaponAttribute.LowerStatReq,			10, 100, 10 );
						i--; //Does not count as bonus - by Blady
						break;
					...
					case 22: ApplyAttribute( secondary, min, max, AosWeaponAttribute.DurabilityBonus,		10, 100, 10 );
						i--; //Does not count as bonus - by Blady
						break;
		...
		public static SlayerName GetRandomSlayer()
		{
			...
			else
			{
				...
				}while(entry.Name == SlayerName.DaemonDismissal || entry.Name == SlayerName.BalronDamnation);
				// *** ***
			}
		...
		public static void ApplyAttributesTo( BaseArmor armor, bool isRunicTool, int luckChance, int attributeCount, int min, int max )
		{
			...
			//Armor resists applied separately, otherwise the chance for having 5 resist bonuses is too low
			if ( !isShield )
			{
				int resistCount = Utility.Random(attributeCount);
				for (int i = 0; i < resistCount; i++)
				{
					int random = GetUniqueRandom( 5 );
					if ( random == -1 )
						break;
					switch ( random )
					{
						case 0: ApplyResistance( armor,	min, max, ResistanceType.Physical,				1, 15 ); break;
						case 1: ApplyResistance( armor,	min, max, ResistanceType.Fire,					1, 15 ); break;
						case 2: ApplyResistance( armor,	min, max, ResistanceType.Cold,					1, 15 ); break;
						case 3: ApplyResistance( armor,	min, max, ResistanceType.Poison,				1, 15 ); break;
						case 4: ApplyResistance( armor,	min, max, ResistanceType.Energy,				1, 15 ); break;
					}
				}
				attributeCount -= resistCount;
			}

			if ( !isShield && armor.MeditationAllowance == ArmorMeditationAllowance.All )
				m_Props.Set( 3, true ); // remove mage armor from possible properties
			if ( isRunicTool )
				m_Props.Set( isShield ? 4 : 0, true ); // remove Durability Bonus from possible properties
			...
			for ( int i = 0; i < attributeCount; ++i )
			{
				...
				switch ( random )
				{
					...
					case  3: ApplyAttribute( primary,	min, max, AosAttribute.CastSpeed,				1, 1 ); break;
						/* Begin Armor */
					case  4: ApplyAttribute( secondary,	min, max, AosArmorAttribute.DurabilityBonus,	10, 100, 10 );
						if ( !isShield )
							i--; //Not counted in the 5 props limit - by Blady.
						break;
					case  5: ApplyAttribute( secondary,	min, max, AosArmorAttribute.LowerStatReq,		10, 100, 10 );
						if ( !isShield )
							i--; //Not counted in the 5 props limit - by Blady.
						break;
					case  6: ApplyAttribute( secondary,	min, max, AosArmorAttribute.SelfRepair,			1, 5 );
						if ( !isShield )
							i--; //Not counted in the 5 props limit - by Blady.
						break;
						/* End Shields */
					...
					case 11: ApplyAttribute( primary,	min, max, AosAttribute.NightSight,				1, 1 );
						i--; //Not counted in the 5 props limit - by Blady.
						break;
		...
		public void ApplyAttributesTo( BaseHat hat )
		{
			CraftResourceInfo resInfo = CraftResources.GetInfo( m_Resource );
			...
			ApplyAttributesTo( hat, true, 0, attributeCount, min, max);
		}
		...
		public static void ApplyAttributesTo( BaseHat hat, bool isRunicTool, int luckChance, int attributeCount, int min, int max )
		{
			...
			for ( int i = 0; i < attributeCount; ++i )
			{
				...
				switch ( random )
				{
					...
					case  4: ApplyAttribute( primary,	min, max, AosAttribute.NightSight,				1, 1 );
						i--; //Does not count as bonus - by Blady
						break;
					...
					case 11: ApplyAttribute( secondary,	min, max, AosArmorAttribute.LowerStatReq,		10, 100, 10 );
						i--; //Does not count as bonus - by Blady
						break;
					case 12: ApplyAttribute( secondary,	min, max, AosArmorAttribute.SelfRepair,			1, 5 );
						i--; //Does not count as bonus - by Blady
						break;
					case 13: ApplyAttribute( secondary, min, max, AosArmorAttribute.DurabilityBonus,	10, 100, 10 );
						i--; //Does not count as bonus - by Blady
						break;
		...
		public static void ApplyAttributesTo( Spellbook spellbook, bool isRunicTool, int luckChance, int attributeCount, int min, int max )
		{
			...
			for ( int i = 0; i < attributeCount; ++i )
			{
				...
				switch ( random )
				{
					//the max intensity of props on a book is 80% of the max intensity in loot items:
					case  0:
					case  1:
					case  2:
					case  3:
