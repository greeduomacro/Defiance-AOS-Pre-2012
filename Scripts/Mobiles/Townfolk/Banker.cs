...
namespace Server.Mobiles
{
	public class Banker : BaseVendor
	{
		...
		{
			public SendTarget() : base( -1, false, TargetFlags.None )
			{
			}
			...
			protected override void OnTarget( Mobile from, object targeted )
			{
				if ( targeted is Gold )
				{
					...
					if ( !item.IsChildOf( from.Backpack ) )
						from.SendMessage("You can only deposit gold from your backpack.");
					else if ( !from.BankBox.TryDropItem( from, item, false ) )
						from.SendMessage("Your bank box is full.");
					else
						from.SendMessage("The gold was placed in your bank box.");
				} else
					from.SendMessage("You can only deposit gold from your backpack.");
			}
		}
		...
		{
			if ( from.Alive && !SpellHelper.IsFeluccaT2A(from.Map, from.Location) && ( from.Region == null || !from.Region.IsPartOf( "Khaldun" ) ) )
				list.Add( new OpenBankEntry( from, this, !(this is CriminalBanker) ) );
