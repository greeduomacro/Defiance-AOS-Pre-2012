...
namespace Server.Mobiles
{
	...
	public class DemonKnight : BaseCreature
	{
		...
		public override bool AutoDispel{ get{ return true; } } //by Blady
		...
		{
			if ( to == null || to.AccessLevel > AccessLevel.Player || artifact == null )
				return;
		...
		public static int GetArtifactChance( Mobile boss )
		{
			...
			if ( boss is DemonKnight )
				chance = 8000 + (luck / 5); //10% - Default is 1500
			else if ( boss is RoyaltyDefender )
				chance = 1250 + (luck / 8 );
			else
				chance = 750 + (luck / 10);
		...
		[Constructable]
		public DemonKnight() : base( AIType.AI_Necro, FightMode.Closest, 10, 1, 0.2, 0.4 )
		{
		...
		public override void OnDamage( int amount, Mobile from, bool willKill )
		{
			if ( from != null && from != this && !m_InHere )
			{
				...
				m_InHere = false;
				//Teleport ability added by Blady:
		...
		public void TeleportOnPlayer()
		{
			...
			foreach (Mobile m in this.GetMobilesInRange(13))
			{
				if ( m == null || m == this || !CanBeHarmful(m) || !m.Alive || m.AccessLevel > AccessLevel.Player)
					continue;
				else if (m.Player)
					list.Add(m);
			}

			PlayerMobile mob = null;
			double mobSkills = 0;
			foreach (PlayerMobile pm in list)
			{
				double pmSkills = pm.Skills.Wrestling.Value + pm.Skills.Swords.Value + pm.Skills.Macing.Value + pm.Skills.Fencing.Value +
								pm.Skills.Archery.Value + pm.Skills.Parry.Value + pm.Skills.Anatomy.Value;
				if (mob == null)
				{
					mob = pm;
					mobSkills = pmSkills;
				}
				else if (mobSkills > pmSkills && Utility.Random(4) != 0) //Random factor - not always going to the weekest.
				{
					mob = pm;
					mobSkills = pmSkills;
				}
			...
			if ( mob != null )
			{
				...
				this.Location = to;
				this.Combatant = mob;
				Effects.SendLocationParticles( EffectItem.Create( from, mob.Map, EffectItem.DefaultDuration ), 0x3728, 10, 10, 2023 );
				Effects.SendLocationParticles( EffectItem.Create(   to, mob.Map, EffectItem.DefaultDuration ), 0x3728, 10, 10, 5023 );
				mob.PlaySound( 0x1FE );
			}
		}
