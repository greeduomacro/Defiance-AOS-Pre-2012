	...
	public abstract class BaseAI
	{
		...
		private class InternalEntry : ContextMenuEntry
		{
				...
				: base( number, range )
			{
				...
				// *** Added for hirables
				if (!mobile.CanOrder && order != OrderType.Release)
					Enabled = false;
				// ***
			...
			public override void OnClick()
			{
				if ( !m_Mobile.Deleted && m_Mobile.Controlled && m_From.CheckAlive() )
				{
					...
					// *** Added for hirables
					m_Mobile.OnReciveOrder(m_From, m_Order);
					// ***
		...
		public virtual void EndPickTarget( Mobile from, Mobile target, OrderType order )
		{
			...
			// *** Changed for hirables ( Moved some code from here to this function also )
			if ( !m_Mobile.OnTargetOrder( from, target, order ) )
				return;
			// ***
...
*/
			...
			if ( m_Mobile.Controlled && m_Mobile.Commandable )
			{
				...
				if ( e.Mobile.Alive && (isOwner || isFriend) )
				{
					...
					for( int i = 0; i < keywords.Length; ++i )
					{
						...
						// *** Added for hirables ***
						if ( !m_Mobile.OnReciveCommand( e.Mobile, keyword ) )
							return;
						// *** ***
						...
						switch( keyword )
						{
							...
							case 0x16D: // *release
							{
								...
								if ( WasNamed( speech ) && m_Mobile.CheckControlChance( e.Mobile ) )
								{
									...
									{
										// *** Changed for Hirables
										m_Mobile.OnRelease( true );
										// ***
									}
								}
		...
		public virtual bool DoOrderCome()
		{
			if ( m_Mobile.ControlMaster != null && !m_Mobile.ControlMaster.Deleted )
			{
				...
				else
				{
					...
					bool bRun = ShouldRun( m_Mobile, 5, m_Mobile.ControlMaster );
		...
		public virtual bool DoOrderFollow()
		{
			...
			else if ( m_Mobile.ControlTarget != null && !m_Mobile.ControlTarget.Deleted && m_Mobile.ControlTarget != m_Mobile && (m_Mobile.ControlTarget.Alive || m_Mobile.ControlTarget == m_Mobile.ControlMaster) && (!m_Mobile.ControlTarget.Hidden || m_Mobile.ControlTarget == m_Mobile.ControlMaster) && m_Mobile.InRange( m_Mobile.ControlTarget, m_Mobile.RangePerception * 2 ) )
			{
				...
				else
				{
					...
					// Not exactly OSI style, but better than nothing.
					bool bRun = ShouldRun( m_Mobile, 4, m_Mobile.ControlMaster );
		...
		public static bool ShouldRun( BaseCreature creature, int iRange, Mobile mTarget )
		{
			if (creature != null && mTarget != null)
			{
				if (creature.InRange(mTarget.Location, iRange))
				{
					creature.CurrentSpeed = creature.ActiveSpeed;
					return false;
				}
				else
				{
					creature.CurrentSpeed = 0.08;
					return true;
