...
namespace Server.Mobiles
{
	public class Barracoon : BaseChampion
	{
		...
		public Barracoon() : base( AIType.AI_Melee )
		{
			...
			SetSkill( SkillName.Magery, 100.0 );//Sunny edit
			SetSkill( SkillName.EvalInt, 100.0 );//added by Blady
		...
		private class ExpirePolymorphTimer : Timer
		{
			...
			public ExpirePolymorphTimer( Mobile owner ) : base( TimeSpan.FromMinutes( 2.0 + Utility.RandomDouble() ) ) //Edit by Blady - was 3.0 minutes
			{
				m_Owner = owner;
		...
		public void TeleportOnPlayer()
		{
			...
			foreach (PlayerMobile pm in list)
			{
				if (mob == null)
					mob = pm;
				else if (pm.Str < mob.Str && Utility.Random(2) < 1)
					mob = pm;
			}
