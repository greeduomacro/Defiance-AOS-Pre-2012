...
namespace Server.Mobiles
{
	...
	public class BaseCreature : Mobile, IHonorTarget
	{
		...
		public virtual double BonusPetDamageScalar{ get{ return 0.0; } }
		...
		public virtual int BreathComputeDamage()
		{
			...
			if ( IsPlagued ) // Added by Silver
				damage = (int)(damage / Plague.HitsBuff);
			if ( IsElder ) // Added by Silver
				damage = (int)(damage / Elders.HitsBuff);

			return (damage > 120) ? 120 : damage; // Capped by Silver
		}
		#endregion
		...
		public virtual double AuraMinDelay{ get{ return 5.0; } }
		public virtual double AuraMaxDelay{ get{ return 10.0; } }

		public virtual double AuraStallTime{ get{ return 2.0; } }
		public virtual double AuraEffectDelay{ get{ return 1.3; } }
		public virtual double AuraDamageDelay{ get{ return 1.0; } }
		...
		public virtual int AuraAngerSound{ get{ return GetAngerSound(); } }
		public virtual int AuraAngerAnimation{ get{ return 12; } }

		public virtual int AuraDamageMessage{ get{ return 0; } }

		public virtual void AuraStart(Mobile target)
		{
			AuraStallMovement();
			AuraPlayAngerSound();
			AuraPlayAngerAnimation();

			this.Direction = this.GetDirectionTo(target);

			Timer.DelayCall(TimeSpan.FromSeconds(AuraEffectDelay), new TimerStateCallback(AuraEffect_Callback), target);
		}

		public virtual void AuraStallMovement()
		{
			if (m_AI != null)
				m_AI.NextMove = DateTime.Now + TimeSpan.FromSeconds(AuraStallTime);
		}

		public virtual void AuraPlayAngerSound()
		{
			PlaySound(AuraAngerSound);
		}

		public virtual void AuraPlayAngerAnimation()
		{
			Animate(AuraAngerAnimation, 5, 1, true, false, 0);
		}

		public virtual void AuraEffect_Callback(object state)
		{
			Mobile target = (Mobile)state;
			...
			Timer.DelayCall(TimeSpan.FromSeconds(AuraDamageDelay), new TimerStateCallback(AuraDamage_Callback), target);
		}

		public virtual void AuraDamage_Callback(object state)
		{
			Mobile target = (Mobile)state;
			...
			{
				DoHarmful(target);
				AuraDealDamage(target);
			}
		}

		public virtual void AuraDealDamage(Mobile target)
		{
			int physDamage = AuraPhysicalDamage;
			int fireDamage = AuraFireDamage;
			int coldDamage = AuraColdDamage;
			int poisDamage = AuraPoisonDamage;
			int nrgyDamage = AuraEnergyDamage;

			if (physDamage == 0 && fireDamage == 0 && coldDamage == 0 && poisDamage == 0 && nrgyDamage == 0)
			{
				target.Damage(AuraComputeDamage(), this);
			}
			else
			{
				AOS.Damage(target, this, AuraComputeDamage(), physDamage, fireDamage, coldDamage, poisDamage, nrgyDamage);
			}

			if (AuraDamageMessage != 0)
				target.SendLocalizedMessage(AuraDamageMessage, false, this.Name);
		}

		public virtual int AuraComputeDamage()
		{
			return Utility.RandomMinMax(10, 20);
		}
		#endregion
		...
		*/
		...
		// *** Added By Eclipse ***
		public virtual bool CanBeDismounted{ get{ return true; } }

		private bool m_bIgnorePlagueLoot;
		[CommandProperty(AccessLevel.GameMaster)]
		public bool IgnorePlagueLoot
		{
			get { return m_bIgnorePlagueLoot; }
			set { m_bIgnorePlagueLoot = value; }
		}

		private bool m_bIsPlagued;
		[CommandProperty(AccessLevel.GameMaster)]
		public bool IsPlagued
		{
			get { return m_bIsPlagued; }
			set
			{
				if ( IsPlagued == value || IsParagon || IsElder )
					return;
				else if (value)
					Plague.Convert(this);
				else
					Plague.UnConvert(this);
		...
		private bool m_bIsElder;
		[CommandProperty(AccessLevel.GameMaster)]
		public bool IsElder
		{
			get { return m_bIsElder; }
			set
			{
				if (IsElder == value || IsParagon || IsPlagued)
					return;
				else if (value)
					Elders.Convert(this);
				else
					Elders.UnConvert(this);
		...
		#region Hireable Npc
		private bool m_bIsHired;
		public bool IsHired
		{
			get { return m_bIsHired; }
			set { m_bIsHired = value; }
		}

		private bool m_bCanOrder;
		public bool CanOrder
		{
			get { return m_bCanOrder; }
			set { m_bCanOrder = value; }
		}

		public virtual void OnRelease(bool orderFromPlayer)
		{
			ControlTarget = null;
			ControlOrder = OrderType.Release;
		}

		public virtual bool OnReciveCommand(Mobile from, int keyword)
		{
			return true;
		}

		public virtual void OnReciveOrder(Mobile from, OrderType order)
		{
		}

		public virtual bool OnTargetOrder(Mobile from, Mobile target, OrderType order)
		{
			bool isOwner = (from == ControlMaster);
			bool isFriend = (!isOwner && IsPetFriend(from));
			...
			{
				if (target is BaseCreature && ((BaseCreature)target).IsScaryToPets && IsScaredOfScaryThings)
				{
					SayTo(from, "Your pet refuses to attack this creature!");
					return false;
				}
				...
				{
					SayTo(from, "Your pet refuses to attack the guard.");
					return false;
				}
			}
			return true;
		...
		*/
		...
		public override void Damage( int amount, Mobile from )
		{
			...
			if ( Core.AOS && !this.Summoned && this.Controlled && 0.2 > Utility.RandomDouble() )
				bonus += BonusPetDamageScalar; //Reduced by 1.0 for all creatures
			...
			{
				bonus += 0.1;
				int recip = Math.Min( amount, 35 );
				from.Damage( (int)( recip * (bonus - (from.Skills[SkillName.MagicResist].Value / 200.0)) ), from );
			}
		...
		public override void OnBeforeSpawn( Point3D location, Map m )
		{
			if ( Paragon.CheckConvert( this, location, m ) )
				IsParagon = true;
//Added Elders/Plague (Eclipse)
			else if ( Elders.CheckConvert( this, location, m ) )
				IsElder = true;
			else if ( Plague.CheckConvert( this, location, m ) )
				IsPlagued = true;
//End Add
		...
		public virtual void OnCarve( Mobile from, Corpse corpse, Item with )
		{
			...
			// Added !IsPlagued
			if ( (!IsPlagued && feathers == 0 && wool == 0 && meat == 0 && hides == 0 && scales == 0) || Summoned || IsBonded )
			{
			...
			else
			{
				...
				new Blood( 0x122D, 0 ).MoveToWorld( corpse.Location, corpse.Map );

				if ( IsPlagued && !IgnorePlagueLoot )
				{
					corpse.DropItem( new PoisonFungus( BaseCreatureLoot.GetPoisonFungusAmount( Fame ) ) );
					from.SendMessage( "You cut away some poison fungus, but they remain on the corpse." );
				}
		...
		public override void Serialize( GenericWriter writer )
		{
			...
			// Version 15 (Hirables)
			writer.Write( (bool)m_bIsHired );
			writer.Write( (bool)m_bCanOrder );
			writer.Write( (bool)m_bIsElder );

			// Version 16
			writer.Write( (bool)m_bIsPlagued );

			// Version 17
			writer.Write( (bool)m_bIgnorePlagueLoot );
			...
			// Version 20
			writer.Write( (bool)m_bIgnoreRangeSensitive );
			writer.Write( (bool)m_bRunToWayPoint );
			writer.Write( (bool)m_bDestroyObstacles );
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			{
				reader.ReadBool(); //bool m_IsChampionMonster
				reader.ReadInt(); //int m_SpawnLevel
			}

			// Version 15 (Hirables)
			if ( version >= 15 )
			{
				m_bIsHired = reader.ReadBool();
				m_bCanOrder = reader.ReadBool();
				m_bIsElder = reader.ReadBool();
			}

			// *** Plagued
			if ( version >= 16 )
				m_bIsPlagued = reader.ReadBool();

			if ( version >= 17 )
				m_bIgnorePlagueLoot = reader.ReadBool();
			// ***
			...
			{
				reader.ReadBool(); //m_IsChampionMonster
				reader.ReadInt(); //m_SpawnLevel
			}
			// *** ***
			...
			{
				if ( version < 21 )
					reader.ReadBool(); //m_bIsInvasion

				m_bIgnoreRangeSensitive = reader.ReadBool();
				m_bRunToWayPoint = reader.ReadBool();
				m_bDestroyObstacles = reader.ReadBool();
			}
		...
		public void ChangeAIType( AIType NewAI )
		{
			...
			switch ( NewAI )
			{
				...
				case AIType.AI_Stealth:
					...
					break;
				// ***
				case AIType.AI_Paladin: //Opticon
		...
		public bool Tamable
		{
			...
			{
				// ***
				return m_bTamable && !m_Paragon && !m_bIsElder && !m_bIsPlagued;
				// ***
			}
			set
			{
		...
		public override void AddNameProperties( ObjectPropertyList list )
		{
			...
			//***
			if ( IsParagon )
				list.Add(1070722, "(Paragon)");
			else if ( IsElder )
				list.Add(1070722, "(Elder)");
			else if ( IsPlagued )
				list.Add(1070722, "(Plagued)");
			//else if ( IsInvasion )
			//	list.Add( 1070722, "(Invasion Creature)" );
			//***
		}
		...
		{
			//Added by Blady - Monsters in the LL do not drop gold.
			if ( ( SpellHelper.IsFeluccaT2A(this.Map, this.Location) || ( Region != null && Region.IsPartOf( "Khaldun" ) ) ) && Backpack != null )
			{
				Item gold = this.Backpack.FindItemByType(typeof(Gold));
				if (gold != null && !(this is Beetle || this is PackHorse || this is PackLlama || this is HordeMinionFamiliar) ) // Edit by Silver: Pack Animals
				{
					if (this.LastKiller != null)
						LootPackEntry.AddRandomLoot( this.Backpack, Math.Min( 6, Utility.Random( gold.Amount / 50 )), this.LastKiller, 4, 4, 60, 90 );
					PackItem( new VeriteGem( Math.Min( 10, gold.Amount / 50 )));
					gold.Delete();
				}
			}
			...
			if( LastKiller != null && LastKiller.AccessLevel > AccessLevel.Player )
				CommandLogging.WriteLine(LastKiller, String.Format("{0} killed basecreature type: {1} at {2} in {3}", CommandLogging.Format(LastKiller), GetType().Name, Location.ToString(), Map.ToString()));
			...
			if ( !Summoned && !NoKillAwards && !IsBonded && treasureLevel >= 0 )
			{
				if ( m_Paragon && Paragon.ChestChance > Utility.RandomDouble() )
					...
					PackItem( new TreasureMap( treasureLevel, Map.Felucca ) ); //by Blady - all maps should be for felucca
			}
		...
		{
			// Added for custom loot *******************
			BaseCreatureLoot.BCLoot( this, c, m_Spawning );
			//******************************************
...
*/
		...
		public virtual void OnThink()
		{
			...
			// *** Added for Aura ***
			if (HasAura && !Summoned && DateTime.Now >= m_NextAuraTime)
			{
				Mobile target = this.Combatant;

				if (target != null && target.Alive && !target.IsDeadBondedPet && CanBeHarmful(target) && target.Map == this.Map && !IsDeadBondedPet && target.InRange(this, AuraRange) && InLOS(target) && !BardPacified)
					AuraStart(target);

				m_NextAuraTime = DateTime.Now + TimeSpan.FromSeconds(AuraMinDelay + (Utility.RandomDouble() * AuraMaxDelay));
			}
			// ***
		...
		// *** Added by Eclipse ***
		private bool m_bDestroyObstacles;
		public bool DestroyObstacles
		{
			get { return m_bDestroyObstacles; }
			set { m_bDestroyObstacles = value; }
		}

		private bool m_bIgnoreRangeSensitive;
		public bool IgnoreRangeSensitive
		{
			get { return m_bIgnoreRangeSensitive; }
			set { m_bIgnoreRangeSensitive = value; }
		}

		private bool m_bRunToWayPoint;
		public bool RunToWayPoint
		{
			get { return m_bRunToWayPoint; }
			set { m_bRunToWayPoint = value; }
		}
		// *** ***

		// *** Changed by Eclipse ***
		public virtual bool PlayerRangeSensitive{ get{ return !m_bIgnoreRangeSensitive; } }
		// *** ***
