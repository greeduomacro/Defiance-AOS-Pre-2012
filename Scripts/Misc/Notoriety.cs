...
namespace Server.Misc
{
	public class NotorietyHandlers
	{
		...
		public static int MobileNotoriety( Mobile source, Mobile target )
		{
			...
			if (CTFGame.Running)
			{
				CTFPlayerGameData spgd = CTFGame.GameData.GetPlayerData(source);
				if (spgd != null && spgd.InGame)
				{
					CTFPlayerGameData tpgd = CTFGame.GameData.GetPlayerData(target);
					if (tpgd != null && tpgd.InGame)
						return spgd.Team == tpgd.Team ? Notoriety.Invulnerable : Notoriety.Enemy;
				}
			}
			#endregion
			...
			//Added by Sunny for Coward System
			//if (CowardSystem.CheckCoward(source, target))
			//	return Notoriety.CanBeAttacked;			
		...
		public static int GetNotorietyByLevel( int level )
		{
			level = Math.Min( Math.Max( level, -6 ), 6 );
			//level from -6 to 6 randomly
			int random = Utility.Random( 5000 ) * ( level < 0 ? -1 : 1 );
			return ( 5000 * level + random );
		}
