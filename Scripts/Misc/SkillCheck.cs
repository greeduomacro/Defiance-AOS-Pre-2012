...
namespace Server.Misc
{
	public class SkillCheck
	{
		...
		public static bool CheckSkill( Mobile from, Skill skill, object amObj, double chance )
		{
			...
			gc += ( 1.0 - chance ) * ( success ? 0.6 : 0.2); //Chance increased by Blady
			...
			if ( gc < 0.03 )
				gc = 0.03; //Default was 0.01

			if (DateTime.Now.Hour == 9 || DateTime.Now.Hour == 14 ) //Global Power Hours by Blady
				gc *= 1.5;
			else if (from is PlayerMobile && ((PlayerMobile)from).PowerHourStart + TimeSpan.FromHours( 1.0 ) > DateTime.Now) //50% faster skill gain added by Blady.
				gc *= 1.5;
		...
		public static void Gain( Mobile from, Skill skill )
		{
			...
			if ( skill.Lock == SkillLock.Up )
			{
				...
				if ( from.StrLock == StatLockType.Up && (info.StrGain / 5.0) > Utility.RandomDouble())// *** Sunny edit was (info.StrGain / 33.3)
					GainStat( from, Stat.Str );
				else if ( from.DexLock == StatLockType.Up && (info.DexGain / 5.0) > Utility.RandomDouble())// *** Sunny edit was (info.DexGain / 33.3)
					GainStat( from, Stat.Dex );
				else if ( from.IntLock == StatLockType.Up && (info.IntGain / 5.0) > Utility.RandomDouble())// *** Sunny edit was (info.IntGain / 33.3)
