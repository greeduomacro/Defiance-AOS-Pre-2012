...
namespace Server.Misc
{
	public class AutoRestart : Timer
	{
		...
		private static TimeSpan DefaultRestartTime = TimeSpan.FromHours( 9.0 ); // time of day at which to restart
...
//				return;
		...
		private static Point3D[] m_Locs = new Point3D[] { new Point3D( 1432, 1687, 20 ), new Point3D( 2713, 2163, 0 ) };
...
//				new VendStone( 1153, "BagOfReagents", "Bag of {0} Reagents (SERVER WARS)", 0, 100 ).MoveToWorld( m_RegStoneLocs[i], Map.Felucca );
			...
			{
				Point3D loc = m_Locs[i];

				new AnkhNorth().MoveToWorld( loc, Map.Felucca );
				new PaidFullHealer().MoveToWorld( new Point3D( loc.X + 1, loc.Y, loc.Z ), Map.Felucca );
				new VendStone( 1153, "BagOfReagents", "Bag of 50 Reagents (SERVER WARS)", 0, "50" ).MoveToWorld( new Point3D( loc.X + 2, loc.Y, loc.Z ), Map.Felucca );
				new VendStone( 1427, "BagOfNecroReagents", "Bag of 50 Necro Reagents (SERVER WARS)", 0, "50" ).MoveToWorld( new Point3D( loc.X + 3, loc.Y, loc.Z ), Map.Felucca );
				new VendStone( 2101, "Bandage", "50 Bandages (SERVER WARS)", 0, "50" ).MoveToWorld( new Point3D( loc.X + 4, loc.Y, loc.Z ), Map.Felucca );
				new VendStone( 2117, "Arrow", "50 Arrows (SERVER WARS)", 0, "50" ).MoveToWorld( new Point3D( loc.X + 5, loc.Y, loc.Z ), Map.Felucca );
			}
		...
		public class RestartGump : Gump
		{
			public RestartGump( bool prewarning ) : base( 90, 60 )
			{
				Closable = true;
				Disposable = true;
				Dragable = true;
				Resizable = false;
				AddPage(0);
				AddBackground(-1, 0, 225, 102, 2600);
				AddButton(46, 60, 4005, 4006, 1, GumpButtonType.Reply, 0);
				AddLabel(45, 18, 0, "Server Wars");
				AddLabel(45, 38, 0, String.Format("Status: {0}", prewarning? "starting soon" : "active"));
				AddLabel(80, 60, 0, "Explanation");
			}

			public override void OnResponse( NetState sender, RelayInfo info )
			{
				Mobile m = sender.Mobile;
				m.SendMessage( "Any progress made during the server wars will not be saved, server wars will end when the shard restarts." );
			}
		}
	}
}