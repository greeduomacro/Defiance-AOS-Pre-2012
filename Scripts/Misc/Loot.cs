...
namespace Server
{
	public class Loot
	{
		...
		public static Item RandomArmorOrHat( bool inTokuno )
		{
			...
			if ( Core.AOS )
				//By Blady - chance for random armor should be higher than chance for hats.
				return Construct( m_ArmorTypes, m_ArmorTypes, m_ArmorTypes, m_AosHatTypes, m_HatTypes );
		...
		public static void AddGems( Container cont, int amount )
		{
			for ( int i = 0; i < amount; i++ )
				AddSortedItem(Loot.RandomGem(), cont);
		}

		public static void AddPotions( Container cont, int amount )
		{
			for ( int i = 0; i < amount; i++ )
				cont.DropItem( RandomPotion() );
		}

		public static void AddScrolls( Container cont, int amount, bool necro )
		{
			for ( int i = 0; i < amount; i++ )
				AddSortedItem( RandomScroll(0, necro ? 15 : 62, necro ? SpellbookType.Necromancer : SpellbookType.Regular ), cont );
		}

		public static void AddRegs( Container cont, int amount, bool necro )
		{
			for ( int i = 0; i < amount; i++ )
				AddSortedItem( necro ? RandomNecromancyReagent() : RandomReagent(), cont );
		}
