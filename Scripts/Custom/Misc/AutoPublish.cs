		...
		public void Check()
		{
			...
			if ( File.Exists( AutoPublish.PublishCFG ) )
			{
				...
				if ( m_Force || pubrev > AutoPublish.PublishRevision ) //New code to push!
				{
					...
					string fwcompiler = @"C:\Windows\Microsoft.NET\Framework\v2.0.50727\csc.exe";
					string defines = String.Empty;
		#if Framework_3_5
					fwcompiler = @"C:\Windows\Microsoft.NET\Framework\v3.5\csc.exe";
					defines = "/d:Framework_3_5";
		#elif Framework_4_0
					fwcompiler = @"C:\Windows\Microsoft.NET\Framework\v4.0.21006\csc.exe";
					...
					compileinfo.Arguments = String.Format( @"/unsafe /out:{1} /recurse:Server\*.cs /win32icon:Server\runuo.ico /warnaserror+ {0}", defines, serverpath );
