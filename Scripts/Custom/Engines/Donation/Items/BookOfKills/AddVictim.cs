	...
	public class KillBookSystem
	{
		...
		public static void Killed( Mobile m ) //done
		{
			...
			for ( int i = 0; i < owner.Aggressors.Count; ++i )
			{
				...
				else if ( info.Defender.Player && (DateTime.Now - info.LastCombatTime) < lastTime && owner.LastKiller == info.Defender ) //added by Blady - it should count kills of defenders as well.
				{
