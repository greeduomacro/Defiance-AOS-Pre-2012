...
namespace Server.Gumps
{
	public class FriendListGump : Gump
	{
			...
			: base( 20, 20 )
		{
			...
			this.AddPage(0);
			this.AddBackground(0, 0, 240, 315, 9200);
			AddAlphaRegion( 0, 0, 240, 315 );
			AddLabel(15, 10, 133, String.Format( "Defiance friend list" ) );
			AddButton( 165, 10, 2031, 2032, 9, GumpButtonType.Reply, 0 ); // help
			this.AddLabel(20, 35, 133, String.Format( "Friends of {0}", m.Name ) );
			this.AddLabel(20, 60, 133, "Name");
			this.AddLabel(150, 60, 133, "Tell");
			this.AddLabel(185, 60, 133, "Remove");

			//next page
			if ( friends.Count - 8 * m_Page - 7 > 0 )
				AddButton(180, 35, 9903, 9905, 2, GumpButtonType.Reply, 0);

			//last page
			if ( m_Page > 0 )
				AddButton(155, 35, 9909, 9911, 1, GumpButtonType.Reply, 0);

			this.AddLabel(160, 285, 133, "Close");
			this.AddButton(195, 285, 4023, 4025, 0, GumpButtonType.Reply, 0);
			this.AddLabel(20, 285, 133, "Add Friend");
			this.AddButton(90, 285, 4014, 4016, 5, GumpButtonType.Reply, 0);

			for ( int i = 8 * m_Page; i >= 0 && i < friends.Count && 8 * m_Page + 8 - i > 0; i++ )
			{
				Mobile friend = (Mobile)friends[i];
				AddLabel( 20, 80 + 25 * (i % 8), friend.NetState != null ? 80 : 2401, friend.Name );
				AddButton( 150, 80 + 25 * (i % 8), 4014, 4016, 110 + i, GumpButtonType.Reply, 0 );
				AddButton( 185, 80 + 25 * (i % 8), 4017, 4019, 10 + i, GumpButtonType.Reply, 0 );
			}
		}
