...
namespace Server.Engines.Jail
{
	...
	public class JailSystem
	{
		...
		/// <summary>
		/// AccessLevel needed to jail/unjail and view the jail roaster
		/// </summary>
		...
		/// from here.
		/// </summary>
		private static Point3D m_JailExit = new Point3D( 5278, 1185, 0 );
		/// <summary>
		...
		/// </summary>
		private static string m_BackupFolder = @"Jail\Backup";
		/// <summary>
		/// The folder for jail purges, relative to the base runuo folder
		/// </summary>
		private static string m_PurgeFolder = @"Jail\Purged";
		...
		{
			CommandLogging.WriteLine( from, "{0} {1} using [jail on player {2}", from.AccessLevel, CommandLogging.Format( from ), CommandLogging.Format( target ) );
		...
		private static void OnJailInfoCommand( CommandEventArgs e )
		{
			...
			{
				e.Mobile.SendMessage( "Looking for trouble?" );
			}
		}
