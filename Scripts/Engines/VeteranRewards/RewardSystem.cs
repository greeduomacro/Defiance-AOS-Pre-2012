...
using Server.Events.CTF; // Silver
...
namespace Server.Engines.VeteranRewards
{
	public class RewardSystem
	{
		...
		public static TimeSpan RewardGameTimeInterval = TimeSpan.FromHours( 20 );
		...
		{
			// ***
			if (item is EtherealMount && ((EtherealMount)item).IsDonationItem)
				return true;
			// ***
			...
			m_Lists = new RewardList[]
				{
					...
					new RewardList( RewardInterval, RewardGameTimeInterval, 2, new RewardEntry[]
					{
						...
						new RewardEntry(	houseAddOns, "Banner Deed", typeof( BannerOSIDeed ) ),
						new RewardEntry(	houseAddOns, "Flaming Skull Deed", typeof( FlamingHeadOSIDeed ) )
					} ),
					...
					new RewardList( RewardInterval, RewardGameTimeInterval, 4, new RewardEntry[]
					{
						...
						new RewardEntry(	houseAddOns, 1049737, typeof( DecoShieldDeed ) ),
						new RewardEntry(	houseAddOns, 1049738, typeof( HangingSkeletonOSIDeed ) )
					} ),
					new RewardList( RewardInterval, RewardGameTimeInterval, 5, new RewardEntry[]
					{
						...
						new RewardEntry(	houseAddOns, 1049739, typeof( AnkhDeed ) )
					} ),
		...
		private static void EventSink_Login( LoginEventArgs e )
		{
			...
			if ( e.Mobile.SkillsCap == 7000 || e.Mobile.SkillsCap == 7050 || e.Mobile.SkillsCap == 7100 || e.Mobile.SkillsCap == 7150 || e.Mobile.SkillsCap == 7200 )
			{
				...
				if ( SkillCapRewards )
					e.Mobile.SkillsCap = 7200; //7000 + (level * 50);
				else
					e.Mobile.SkillsCap = 7200; // Silver edit: Everyone gets 720 skill points
			}

			if ( cur < max && !e.Mobile.Region.IsPartOf( typeof( CTFGameRegion ) ) ) // Silver: No claiming donations in CTF
