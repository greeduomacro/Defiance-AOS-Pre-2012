...
namespace Server.Factions
{
	public class PlayerState : IComparable
	{
		...
		public DateTime GetGiftDate( Mobile mob) //by Blady
		{
			if ( m_SilverGiven == null )
			...
			for ( int i = 0; i < m_SilverGiven.Count; ++i )
			{
				...
				if ( sge.IsExpired )
					...
					return sge.TimeOfGift;
			}
			return DateTime.MinValue;
		}

		public bool CanGivePointsTo ( Mobile mob) //by Blady
		{
			PlayerState otherState = PlayerState.Find( mob );
			return ( GetGiftDate(mob) <= otherState.GetGiftDate(m_Mobile) );
		}
		...
		public void OnGivenSilverTo( Mobile mob )
		{
			if ( m_SilverGiven != null )
			{
				...
				{
					SilverGivenEntry sge = m_SilverGiven[i];
					if ( sge.GivenTo == mob || sge.IsExpired )
						m_SilverGiven.RemoveAt( i-- );
				}
			}
			else
