...
namespace Server.Factions
{
	...
	public abstract class Faction : IComparable
	{
		...
		//*** Edit by Sunny ***\\ from 7.0
		public static readonly TimeSpan LeavePeriod = TimeSpan.FromDays(2.0);
		...
		public void BeginHonorLeadership( Mobile from )
		{
			...
			from.SendMessage("This option has been disabled. You cannot transfer kill points.");
		}
		...
		public void JoinGuilded( PlayerMobile mob, Guild guild )
		{
			...
			else if ( mob.SkillsTotal < 7000 ) //Added by Blady
			{
				guild.RemoveMember( mob );
		...
		public bool HasEnoughSkills( Mobile mob )
		{
			...
			return mob.SkillsTotal >= 7000 && gms >= 5 && totalStats >= 225;
		}
		...
		public static readonly TimeSpan SkillLossPeriod = TimeSpan.FromMinutes( 3.0 );
		...
		public const int StabilityActivation = 40; // Stablity code goes into effect when largest faction has > 200 people
		...
		public static void HandleDeath( Mobile victim, Mobile killer )
		{
			...
			if ( pack != null )
			{
				...
				for ( int i = 0; i < sigils.Length; ++i )
				{
					...
					//Added by Blady for score gaining when capturing towns:
					sigil.CanGainPoints = false;
			...
			if ( !IsFactionFacet( killer.Map ) ) // *** Edit by Sunny
				return;
			...
			if ( killerState.Faction != victimState.Faction )
			{
				...
				else
				{
					...
					if ( victimState.CanGivePointsTo( killer ) ) //By Blady
					{
						if ( victimState.KillPoints > 0 && victimState.CanGiveSilverTo( killer ) )
						{
							//*** Edit by Sunny ***\\
							int reward = award * 40;

							for ( int i = 0; i < Town.Towns.Count; ++i )
								if ( Town.Towns[i].Owner == victimState.Faction )
									reward += 50;

							if ( reward > 0 )
							{
								int silver = killerState.Faction.AwardSilver( killer, reward );
		...
		// *** ***
		public static readonly Map BaseFacet = Map.Felucca;
		private static readonly List<Map> Facets = Map.AllMaps;

		public static bool IsFactionFacet( Map map )
		{
			if ( map != Map.Trammel && map != Map.Malas )
				return Facets.Contains( map );
