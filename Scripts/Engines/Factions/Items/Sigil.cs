...
namespace Server.Factions
{
	public class Sigil : BaseSystemController
	{
		...
		public static readonly TimeSpan CorruptionPeriod = ( (Core.AOS) ? TimeSpan.FromHours( 10.0 ) : TimeSpan.FromHours( 24.0 ) );
		...
		public static readonly TimeSpan PurificationPeriod = TimeSpan.FromDays( 7.0 );

		//Added by Blady for score gainign when capturing towns:
		public bool CanGainPoints = false;
			...
			if ( obj is Mobile )
			{
				if ( obj is PlayerMobile )
				{
					...
					else if ( from.NetState != null && targ.NetState != null )
					{
						...
						{
							pack.DropItem( this );
							CanGainPoints = false; //added by Blady
						}
					}
				}
				else
			...
			else if ( obj is BaseMonolith )
			{
				...
				if ( obj is StrongholdMonolith )
				{
					...
					else
					{
						...
						PlayerState plState = PlayerState.Find( from );
						...
						if ( oldController == null )
						{
							if ( m_Corrupted != newController )
							{
								...
								/*added by Blady // Disabled by Silver
								if (CanGainPoints && plState != null)
								{
									plState.KillPoints += 1;
									from.SendMessage("You have gained 1 faction point for delivering the sigil to the Monolith!");
								}*/
							}
						}
						else if ( m_GraceStart > DateTime.MinValue && (m_GraceStart + CorruptionGrace) < DateTime.Now )
						{
							if ( m_Corrupted != newController )
							{
								...
								{
									plState.KillPoints += 1;
									from.SendMessage("You have gained 1 faction point for returning the sigil to the Monolith!");
								}*/
							}
							else
								ClearCorrupting();
				...
				else if ( obj is TownMonolith )
				{
					...
					else
					{
						...
						if (CanGainPoints && plState != null)
						{
							if( plState.KillPoints < 20 )// Edit by Silver: Added cap of 20
							{
								plState.KillPoints += 1; // Edit by Silver: Changed 2 to 1
								from.SendMessage("You have gained 1 faction point for gaining the control over the town!");
							}

							// Added by Silver
							int silver = plState.Faction.AwardSilver( from, 200 );
							if ( silver > 0 )
							{
								from.SendMessage( String.Format( "You have earned {0} silver for gaining the control over the town!", silver) );
							}
						}
					}
				}
