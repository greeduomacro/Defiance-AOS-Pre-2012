		...
		private class TeleportTimer : Timer
		{
			...
			protected override void OnTick()
			{
				...
				else
				{
					...
					if ( m_Mobile.Region.IsPartOf( typeof( Server.Regions.Jail ) ) )
					{
						m_Mobile.SendLocalizedMessage( 1041530, "", 0x35 ); // You'll need a better jailbreak plan then that!
						return;
					}
					if ( !m_Mobile.Region.CanUseStuckMenu( m_Mobile ) )
					{
						m_Mobile.SendMessage("You cannot use the 'Stuck' option here.");
						return;
					}
