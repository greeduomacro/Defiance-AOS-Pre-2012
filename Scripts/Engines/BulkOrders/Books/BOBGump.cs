			...
			for ( int i = index; i < (index + count) && i >= 0 && i < list.Count; ++i )
			{
				...
				if ( obj is BOBLargeEntry )
				{
					...
					for ( int j = 0; j < e.Entries.Length; ++j )
					{
						...
						// ***
						if ( e.DeedType == BODType.Hunter && sub.ItemType != null ) //ItemType is not supposed to be null, unless a class is removed :)
							AddHtml(103, y, 130, 32, HtmlUtility.Color(sub.ItemType.Name, HtmlUtility.HtmlWhite), false, false);
						else
						// ***
				...
				else if ( obj is BOBSmallEntry )
				{
					...
					// ***
					if ( e.DeedType == BODType.Hunter && e.ItemType != null ) //ItemType is not supposed to be null, unless a class is removed :)
						AddHtml(103, y, 130, 32, HtmlUtility.Color(e.ItemType.Name, HtmlUtility.HtmlWhite), false, false);
					else
					// ***
