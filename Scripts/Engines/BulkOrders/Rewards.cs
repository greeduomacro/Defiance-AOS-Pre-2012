...
namespace Server.Engines.BulkOrders
{
	...
	public sealed class SmithRewardCalculator : RewardCalculator
	{
		...
		{
			if ( type >= 1 && type <= 8 )
				return new RunicHammer( CraftResource.Iron + type, Core.AOS ? ( 60 - (type*5) ) : 50 ); // Silver: used to  be 55 - (type*5)
	...
	public sealed class TailorRewardCalculator : RewardCalculator
	{
		...
		{
			if ( type >= 1 && type <= 3 )
				return new RunicSewingKit( CraftResource.RegularLeather + type, 65 - (type*15) );
