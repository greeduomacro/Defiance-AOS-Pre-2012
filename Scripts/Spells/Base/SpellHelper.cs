...
namespace Server.Spells
{
	...
	{
		private static TimeSpan AosDamageDelay = TimeSpan.FromSeconds( 1.32 );
		...
		public static bool ValidIndirectTarget( Mobile from, Mobile to )
		{
			...
			if( fbc != null )
			{
				if( fbc.Controlled || fbc.Summoned )
				{
					...
					return true; //Added by Blady - all wild monsters will harm innocent players.
			}
		...
		private static TravelValidator[] m_Validators = new TravelValidator[]
			{
				...
				new TravelValidator( IsTrammel ),//Edit by Sunny
		...
		private static bool[,] m_Rules = new bool[,]
			{
					/*T2A(Fel)	Ilshenar	Trammel,	Wind(Fel),	Dungeons(Fel),	Solen(Tram),	Solen(Fel),	CrystalCave(Malas),	Gauntlet(Malas),	Gauntlet(Ferry),	Stronghold,		ChampionSpawn,	Dungeons(Tokuno[Malas]), LampRoom(Doom),	GuardianRoom(Doom) */
/* Recall From */	{ false,	true,		true,		false,		false,			true,			false,			false,				false,				false,				true,			true,			true,					false,				false },
/* Recall To */		{ false,	true,		false,		false,		false,			false,			false,			false,				false,				false,				false,			false,			false,					false,				false },
/* Gate From */		{ false,	true,		false,		false,		false,			false,			false,			false,				false,				false,				false,			false,			false,					false,				false },
/* Gate To */		{ false,	true,		false,		false,		false,			false,			false,			false,				false,				false,				false,			false,			false,					false,				false },
/* Mark In */		{ false,	true,		false,		false,		false,			false,			false,			false,				false,				false,				false,			false,			false,					false,				false },
...
/* Tele To */		{ true,		true,		true,		true,		true,			true,			true,			false,				true,				false,				false, 			true,			true,					true,				true },
		...
		public static bool IsInTheLL( Map map, Point3D loc ) //added by Blady
		{
			int x = loc.X, y = loc.Y;

			return (map == Map.Felucca &&
					((x >= 5110 && y >= 2285 && x < 6160 && y < 4110) || (x >= 5370 && y >= 1270 && x < 5640 && y < 1520)));
		}
		...
		private class SpellDamageTimerAOS : Timer
		{
				...
				: base( delay )
			{
				...
				m_DFA = dfa;
				m_Spell = s;
				if( m_Spell != null && m_Spell.DelayedDamage && !IsNotPVP( from, target ) && !m_Spell.DelayedDamageStacking )
					m_Spell.StartDelayedDamageContext( target, this );

				Priority = TimerPriority.TwentyFiveMS;
			}

			private static bool IsNotPVP( Mobile from, Mobile target )
			{
				return !( from is PlayerMobile && target is PlayerMobile );
			}
