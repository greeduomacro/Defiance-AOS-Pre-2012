...
namespace Server.Spells.Sixth
{
	public class InvisibilitySpell : MagerySpell
	{
		...
		public void Target( Mobile m )
		{
			...
			else if ( m is Mobiles.BaseVendor || m is Mobiles.PlayerVendor || m is Mobiles.PlayerBarkeeper || m.AccessLevel > Caster.AccessLevel || CTFGame.GetFlag( m ) != null )
			{
			...
			else if ( CheckBSequence( m ) )
			{
				...
				TimeSpan duration = TimeSpan.FromSeconds( (( 1.2 * Caster.Skills.Magery.Fixed) / 10 ) + Utility.Random( 4 ) );
