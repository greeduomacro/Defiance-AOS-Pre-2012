...
namespace Server.Multis
{
	public abstract class BaseHouse : BaseMulti
	{
		...
		public virtual DecayType DecayType
		{
			get
			{
				...
				for ( int i = 0; i < acct.Length; ++i )
				{
					...
					if ( mob != null && (mob.AccessLevel >= AccessLevel.GameMaster || (mob is PlayerMobile && ((PlayerMobile)mob).HasDonated))  )
						return DecayType.Ageless;
				}
		...
		public void KickPets( Mobile m ) // Added by Silver
		{
			foreach( Mobile mob in Region.GetMobiles() )
			{
				if( mob is BaseCreature && ((BaseCreature)mob).ControlMaster == m )
					mob.Location = BanLocation;
			}
		}

		public bool AreTherePetsOwnedBy( Mobile m ) // Added by Silver
		{
			foreach( Mobile mob in Region.GetMobiles() )
			{
				if( mob is BaseCreature && ((BaseCreature)mob).ControlMaster == m)
...
*/
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			if ( !CheckDecay() )
			{
				...
				if ( m_Owner == null && m_Friends.Count == 0 && m_CoOwners.Count == 0 && !m_RestrictDecay )
