...
namespace Server
{
	...
	public class Mobile : IEntity, IHued, IComparable<Mobile>, ISerializable
	{
		...
		public virtual bool IsNaked()
		{
			...
			{
				if ( /*item.Layer == Layer.Hair || item.Layer == Layer.FacialHair ||*/ item.Layer == Layer.Backpack || item.Layer == Layer.Bank )
					continue;
				return false;
			}
		...
		public static string[] GuildTypes = new string[]
			{
				"",
				" (Chaos)",
				" (Order)"
